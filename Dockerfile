...
RUN apt-get install -y --no-install-recommends ca-certificates lib32gcc1 lib32stdc++6 libcurl4-gnutls-dev:i386 wget tar supervisor
...
RUN groupadd dst
RUN useradd -g dst -d /data dst
RUN chown -R dst:dst /data

# install steamcmd
RUN mkdir -p /opt/steamcmd
RUN wget "${STEAMCMD_URL}" -O /tmp/steamcmd.tar.gz
RUN tar -xvzf /tmp/steamcmd.tar.gz -C /opt/steamcmd
RUN rm -rf /tmp/*
...
RUN steamcmd +runscript /opt/steamcmd_scripts/install_dst_server_initial
RUN rm -rf /root/Steam /root/.steam
